...
namespace App\Modules\RelatedArticlesSeacher;




class RelatedArticlesSeacher
{
    public $count;
    public $out = array();
    ...
    public function get_minification_array($text)
    {
        ...
        $words = array_diff($words, $stop_slova);
    ...
    public function searchSimilarArticle($text, $all_articles)
    {
        $this->count = count($text);
        
        // Проход по всем статьям в таблице
        foreach ($all_articles as $article) {
            $verifiable = $this->get_minification_array($article->title . ' ' . $article->post . ' ' . $article->post_description);
            $similar_counter = 0;
...
//            $this->out[$article->id . ', ' . $article->title] = $similar_counter * 100 / $this->count;
            $this->out[$article->id] = $similar_counter * 100 / $this->count;
        }
        // Сортировка результатов и ограничение до 4шт
        arsort($this->out);
        $this->out = array_slice($this->out, 1, 4, true);
        return $this->out;
    }
}
